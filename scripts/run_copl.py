...
class Config:
    ...
    test_driver_names: list = field(default_factory=lambda: [
        "강신길"
    ])
    ...
    load_vae: str = "test"    # similarity encoder 로드 경로(None=학습, "test"/"20250101_120000"=로드)
    ...
    ctx_obs_only_patience: int = 20

    # CNN specific
    rm_kernel_size: int = 3
    rm_layers: int = 2
    # Preference Transformer specific
    rm_num_heads: int = 8
    rm_max_len: int = 1000

    # Test-time adaptation
    adapt_topk: int = knn_k
