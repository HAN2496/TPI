...
def load_all_drivers(driver_names, features, around, downsample):
    data = Dataset("datasets")
    view = View(features=tuple(features), around=around, downsample=downsample)
    all_data = {}

    for driver_name in driver_names:
        X, y = view(data[driver_name])
...
def analyze_gamma_sensitivity(all_data, gammas=[0.0001, 0.001, 0.01, 0.1, 1.0],
                               save_path="outputs/lab/copl/rkhs_gamma_sensitivity.png"):
    X_sample = []
    y_sample = []
...
def main():
    import os
    os.makedirs("outputs/lab/copl", exist_ok=True)
    ...
    # 1. Load Data
    print(f"\n[1] Loading data (around={AROUND}, features={len(FEATURES)})...")
    all_data = load_all_drivers(DRIVER_NAMES, FEATURES, AROUND, DOWNSAMPLE)
    ...
    plot_full_similarity_matrix(K_default, labels, None, "outputs/lab/copl/rkhs_default_gamma.png")
    ...
    plot_full_similarity_matrix(K_median, labels, median_gamma, "outputs/lab/copl/rkhs_median_gamma.png")
