...
| 구분 | 기호 | 값 | 출처 |
|---|---|---|---|
| 고정 | $l_f + l_r$ | 2.90 m | GV60 공개 제원 (축거). 데이터의 `Vehicle: JW` = GV60 |
| 고정 | $l_f, l_r$ 각각 | 1.45, 1.45 m | 가정: 앞뒤 무게 배분 50:50. E-GMP 는 "이상적 배분" 이라고만 공개, 수치 미공개 |
| 고정 | $m$ | 2,300 kg | GV60 AWD 공차 2,095 kg (공개 제원) + 탑승 3인 약 225 kg (info 파일 `Passengers: 3`) |
| 고정 | $I_{yy}$ | $m\,l_f l_r$ ≈ 4,840 kg·m² | 가정: dynamic index $I_{yy}/(m l_f l_r) = 1$ (승용차 전형값 0.9–1.1). 제원 미공개 |
| 고정 | $\ell$ | −0.29 m | 실측: 학습 데이터에서 휠속 앞뒤 차이 $\Delta v_w$ 를 라벨 $q$ 에 회귀한 기울기 (methods.md §5.8 서두) |
| 고정 | $\omega_c$ | 2π·0.77 rad/s | 실측: `Bounce_rate_6D` 가 "a_z 를 1차 고역통과 후 적분" 과 corr 0.92 가 되는 모서리 주파수 (§5.8-15). 자유로 풀어도 0.75 Hz 로 재현 (§5.8-20) |
| 고정 | $g$ | 9.81 m/s² | 상수. $\theta$ 스케일 앵커 |
| 자유 (차량) | $k_f, k_r$ | 축별 휠레이트 [N/m], 범위 10–400 kN/m | 범위는 2.3 t 차량의 ride 진동수 0.5–3 Hz 에 해당 |
| 자유 (차량) | $c_f, c_r$ | 축별 댐핑 [N·s/m], 범위 0.3–30 kN·s/m | 범위는 감쇠비 0.01–1 에 해당 |
| 자유 (센서) | $h_I, \kappa$ | §3-2 와 동일 (물리 범위 ±0.5 m, −0.12~−0.04 s) | §3-2 의 근거 그대로 |
| 자유 (잡음) | $q_a, q_p, q_g, q_z, r_w, r_x, r_d, r_z$ | 8개 | 측정잡음 하한 σ_x ≥ 0.02, σ_z ≥ 0.03 m/s² (IMU 유효 잡음) |

제원 출처: Genesis GV60 위키백과 (개발코드 JW, 축거 2,900 mm), evspecifications.com (GV60 Standard AWD 공차 2,095 kg, 앞 74 kW / 뒤 160 kW). 데이터 전 폴더의 info 파일이 `Vehicle: JW`, DBC `JW1_FD-*.dbc` 로 같은 차종임을 확인 (methods.md §5.8-9).
...
| | pitch corr (p10) / RMSE / lag | bounce corr (p10) via 칩 체인 $b$ / lag / 0.4–1.5 Hz 대역 corr | bounce corr via 물리 $\dot z_s$ | NIS | label_logloss |
|---|---|---|---:|---:|---:|
| §3-3 독립 진동자 (pb2_basic) | 0.916 (0.793) / 1.80 deg/s / 0 ms | 0.923 (0.861) / 0 ms / 0.985 | 0.259 | 1.10 | 2.32 |
| **§4-1 half-car (pb2_halfcar)** | **0.917 (0.803) / 1.75 deg/s / +10 ms** | **0.923 (0.861) / 0 ms / 0.985** | 0.143 | 1.10 | 2.31 |

(bounce 는 자유 이득 회귀, 이득 ≈ 10.05 로 두 모델 동일. "물리 $\dot z_s$" 열은 칩 체인을 거치지 않은 차체 bounce 속도를 라벨과 직접 비교한 값으로, 라벨이 칩 출력이라 낮은 것이 정상 — §3 의 0.25–0.36 과 같은 현상.)
