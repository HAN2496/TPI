...
def load_data(driver_name, around, downsample, smooth=False, smooth_cutoff=12.0):
    driver = Dataset("datasets")[driver_name]
    ...
    common_cols = None
    for ep in driver:
        if ep.label is not None:
            cols = set(c for c in ep.signals.df.columns if c not in ignore_cols)
            common_cols = cols if common_cols is None else common_cols & cols
    feature_names = sorted(common_cols)

    view = View(features=tuple(feature_names), around=around, downsample=downsample,
                smooth=(smooth_cutoff, 2) if smooth else None)
    X_np, y_np = view(driver)
...
def main(driver_name, around, downsample, target_features, epochs=30, batch_size=256, n_splits=5, smooth=False, smooth_cutoff=12.0):
    X, y, feature_names = load_data(driver_name, around, downsample, smooth=smooth, smooth_cutoff=smooth_cutoff)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--around", type=float, nargs=2, default=[0, 3])  # event_time 기준 [s]
    ...
    drivers = [args.driver] if args.driver else Dataset("datasets").names

    save_dir = Path("outputs/lab/features")
    save_dir.mkdir(parents=True, exist_ok=True)

    all_ranks = {}  # driver -> {feature -> rank}

    txt = save_dir / "sequential_feature_selection.txt"
    with open(txt, "w", encoding="utf-8") as f, redirect_stdout(f):
        for idx, driver in enumerate(drivers):
            print(f"\n{'='*60}")
            print(f"[{idx+1}/{len(drivers)}] Driver: {driver}")
            print(f"{'='*60}")
            all_ranks[driver] = main(driver, args.around, args.downsample, args.target_features,
