...
def extract_data(name):
    features = []
    labels = []
    for ep in Dataset("datasets")[name]:
        if ep.label is None:
            continue
        states = ep.signals.df
        ...
        labels.append(1 if ep.label else 0)
...
if __name__ == "__main__":
    for name in ["강신길", "박재일", "한규택"]:
        correlation_dir = Path("outputs/lab/features") / name / "correlation"
        correlation_dir.mkdir(parents=True, exist_ok=True)

        txt = correlation_dir / "correlation_results.txt"
        ...
        print(f"Correlation analysis results saved to {txt}")