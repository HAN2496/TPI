...
class PlanarFlow(nn.Module):
    ...
    def forward(self, x):
        def m(x): return F.softplus(x) - 1.0
        def h(x): return torch.tanh(x)
        def h_prime(x): return 1.0 - h(x) ** 2
...
class RadialFlow(nn.Module):
    ...
    def forward(self, x):
        def m(x): return F.softplus(x)
        def h(r): return 1.0 / (a + r)
        def h_prime(r): return -h(r) ** 2
