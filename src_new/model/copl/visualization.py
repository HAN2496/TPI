...
def plot_vae_latent_by_label(Z_train, item_labels, save_path, seed=42):
    n = Z_train.shape[0]
    perp = min(30, max(5, (n - 1) // 3))
    z2 = TSNE(n_components=2, random_state=seed, perplexity=perp).fit_transform(Z_train)
    z_pca = PCA(n_components=2).fit_transform(Z_train)

    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    for proj, ax, title in [(z2, axes[0], "t-SNE"), (z_pca, axes[1], "PCA")]:
        m0, m1 = item_labels == 0, item_labels == 1
        ax.scatter(proj[m0, 0], proj[m0, 1], s=15, alpha=0.5, color="steelblue", label="Bad(0)")
        ax.scatter(proj[m1, 0], proj[m1, 1], s=15, alpha=0.5, color="tomato", label="Good(1)")
        ax.legend(fontsize=9)
        ax.set_title(f"VAE Latent Space by Label [{title}]")
        ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
