...
class TSFreshExperiment(BaseExperiment):
    ...
    def train(self, out_dir: Path) -> dict:
        print("[2] Fitting TSFresh pipeline...")
    ...
    def evaluate(self, out_dir: Path) -> dict:
        cfg = self.cfg
        print(f"[3] Evaluating on {cfg.test_driver_name} ({cfg.k_folds}-fold CV)...")
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(Z_test, y_test)):
            ...
            if cfg.verbose > 0:
                print(f"  Fold {fold+1}: AUROC={roc_auc_score(y_test[val_idx], oof_probs[val_idx]):.4f}")
        ...
        print(f"  OOF AUROC={m['auroc']:.4f}  AUPRC={m['auprc']:.4f}  Brier={m['brier']:.4f}")
