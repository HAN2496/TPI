...
$$A_{ii}[i, j] = \begin{cases} \text{sim}(i, j) & \text{if } j \in \text{TopK}(i) \\ 0 & \text{otherwise} \end{cases}$$
...
**Item Update (`gcf_m_i_type="f"`, 현재 설정):**
...
$$m_i^{ii,(l)} = \sum_{j \in N_i^{ii}} \beta_{i,j} \left[ w_{ii} \left( W_i^5 e_j^{(l-1)} + W_i^6 \left( e_j^{(l-1)} \odot e_i^{(l-1)} \right) \right) + W_i^7 \tilde{m}_j^{+} + W_i^8 \tilde{m}_j^{-} \right]$$

where $\tilde{m}_j^{+} = \sum_{u \in N_j^+} \alpha_{u,j}^+ e_u^{(l-1)}$, $\tilde{m}_j^{-} = \sum_{u \in N_j^-} \alpha_{u,j}^- e_u^{(l-1)}$ (item-item 이웃을 평가한 타 유저 임베딩)

$$m_i^{0,(l)} = W_i^0 e_i^{(l-1)} \quad \text{(self-connection)}$$

$$e_i^{(l)} = \text{LeakyReLU}\!\left( m_i^{0,(l)} + m_i^{+,(l)} + m_i^{-,(l)} + \lambda_{ii} \cdot m_i^{ii,(l)} \right)$$

**핵심**:
- $\odot$: Hadamard product (element-wise)
- Positive/Negative 그래프를 **분리**하여 대조 학습
- Item-item 그래프로 content-based 정보 보완 (가중치 $\lambda_{ii}$)
- `gcf_m_i_type="f"`: item-item 이웃의 **콘텐츠** ($w_{ii}$ 가중) + 그 이웃을 평가한 **타 유저 정보**를 동시에 흡수 → 우리 세팅(운전자별 고유 궤적)에서 cross-driver 신호가 item-item 그래프를 통해서만 흐르므로, 이 경로를 강화하기 위한 설계

### Item Update Variants

| `m_i_type` | user→item | item-item (content) | item-item (cross-user) |
|---|---|---|---|
| `a` | ✓ | ✗ | ✗ |
| `b` | ✓ | ✓ | ✗ |
| `c` | ✓ | ✓ | ✓ |
...
`w_ii` (`gcf_loss_kwargs["w_ii"]`): item-content 경로와 cross-user 경로의 상대적 가중치
...
$$v[j] = \sum_{i: j \in N_i} \begin{cases} w_i[j] & \text{if } y_i = 1 \\ -\alpha_{\text{neg}} w_i[j] & \text{if } y_i = 0 \end{cases}$$
