...
class CoPLGCFTrainer:
    ...
    def evaluate(self, va_u_t, va_i_t, va_y_np):
        ...
        return val_loss.item(), roc_auc_score(va_y_np, val_prob)
...
class CoPLRMTrainer:
    ...
    def evaluate(self, loader, E_u_train):
        ...
        return val_auc, total_loss / max(1, n_seen)
    ...
    def train(self, train_loader, val_loader, E_u_train, tr_y, verbose=1):
        ...
        for epoch in range(n_epochs):
            ...
            if verbose > 0 and (epoch % 5 == 0 or epoch == n_epochs - 1):
                print(f"  [RM ] epoch={epoch:03d} train_loss={train_loss:.4f} val_loss={val_loss:.4f} val_auc={val_auc:.4f} best={self.best_auc:.4f}")
