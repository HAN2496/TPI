...
def plot_vae_reconstruction(vae, item_series, feature_names, mu_stats, sd_stats, save_path, n_samples=6, seed=42):
    ...
    vae.eval()
    with torch.no_grad():
        x_hat = vae(X_tensor.to(device))[0].cpu()  # (n, D, T)
...
def plot_vae_latent(Z_train, item_owner_uid, train_drivers, save_path, seed=42):
    ...
    for uid, name in enumerate(train_drivers):
        ...
        plt.scatter(z2[m, 0], z2[m, 1], s=15, alpha=0.6, label=name)
    plt.legend(fontsize=8)
    plt.title('VAE Latent Space (t-SNE, by driver)')
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
def plot_vae_feature_mse(vae, item_series, feature_names, mu_stats, sd_stats, save_path):
    ...
    vae.eval()
    with torch.no_grad():
        x_hat = vae(X_tensor.to(device))[0].cpu().numpy()  # (N, D, T)

    orig = X_tensor.numpy()  # (N, D, T)
    mse_per_feature = ((orig - x_hat) ** 2).mean(axis=(0, 2))  # (D,)
