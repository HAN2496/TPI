...
from .rm import RAPRewardModel
from .trainer import RAPTrainer
...
class RAPExperiment(BaseExperiment): # 클래스 이름 수정
    ...
    def train(self, out_dir: Path) -> dict:
        ...
        plot_training_curves(dict(metrics), out_dir / "plots" / "training_curves.png", title="RAP Training")
        return {}
    ...
    def evaluate(self, out_dir: Path) -> dict:
        ...
        # Train 데이터를 통째로 넘겨서 Memory Bank 소스로 구성
        full_x, full_y, full_uids = [], [], []
        for uid, (item_ids, labels) in self.per_user_items.items():
            full_x.append(self.item_series[item_ids])
            full_y.append(labels)
            full_uids.append(np.full(len(labels), uid))

        full_x = np.concatenate(full_x)
        full_y = np.concatenate(full_y)
        full_uids = np.concatenate(full_uids)

        # adapt_sequential에 풀 데이터 전달
        ctx_sizes, test_aurocs, final_probs, holdout_y, snapshots = self.trainer.adapt_sequential(
            X_test, y_test, full_x, full_y, full_uids
        )
        ...
        if final_probs is not None:
            ...
            for pct, probs in snapshots.items():
                ...
                evaluate_predictions(holdout_y, probs, snap_dir, "metrics",
                                     title=f"RAP - {cfg.test_driver_name} (Context {pct}%)")

            m = evaluate_predictions(holdout_y, final_probs, out_dir / "plots", cfg.test_driver_name,
                                      title=f"RAP - {cfg.test_driver_name} (test)")
        ...
        for uid, uname in enumerate(cfg.train_driver_names):
            ...
            # 학습 유저 평가 시에도 동일한 샘플링 로직을 쓰거나 풀 셋트를 써야 함.
            # Memory Bank에 풀셋을 넣으면 메모리 터질 수 있으니 샘플링
            mem_obs, mem_uids, mem_y = self.trainer._sample_memory(full_x, full_y, full_uids, size=1024)

            with torch.no_grad():
                u_probs = torch.sigmoid(self.model(
                    obs=X_u, uids=uids_t, 
                    mem_obs=mem_obs, mem_uids=mem_uids, mem_y=mem_y
                )).cpu().numpy()

            m = evaluate_predictions(labels, u_probs, out_dir / "plots" / "train", uname,
                                      title=f"RAP - {uname} (train)")
    ...
    def load(self, out_dir: Path) -> None:
        self.model.load_state_dict(
            torch.load(out_dir / "best_rap.pt", map_location=self.cfg.device, weights_only=True))
