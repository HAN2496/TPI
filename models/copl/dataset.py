...
def normalize_bipartite_adj(adj, eps=1e-12):
...
class CoPLGraphDataset:
    """train_data: {driver_name: (X [n,T,C], y [n])} — 데이터 로딩은 밖(View)에서."""

    def __init__(self, train_data, cfg, sim_builder=None):
        self.cfg = cfg
        self.train_drivers = list(train_data)
        self.user_to_uid = {name: i for i, name in enumerate(self.train_drivers)}

        item_series_list, item_owner_uid_list = [], []
        self.per_user_items = {}
        item_offset = 0

        for uname, (X, y) in train_data.items():
        ...
        self.n_users = len(self.train_drivers)
    ...
    def norm(self, X):
        """train 통계로 test 시퀀스 정규화."""
        return (X - self.norm_mean) / self.norm_std if self.norm_mean is not None else X
